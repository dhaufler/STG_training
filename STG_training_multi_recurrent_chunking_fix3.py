...
def precompute_states_with_stride(aud_full, base_state, model, flags, roll_out_func):
    """
    Pre-compute RNN states at chunk boundaries for efficient overlapping chunk processing.
    This enables independent processing of overlapping chunks with correct initial states.
    ...
    # Pre-compute states at stride intervals
    current_state = base_state
    current_pos = 0
    
    # Get the RNN layer for state computation (only needed for zero_state)
    ...
    if flags.fix_randomization_for_state_precomputation:
        ...
        # Note: repeat_number will be controlled in the input loading
    else:
        # Use training sigma (potentially randomized)
        sigma_smoothing = get_sigma_smoothing(flags, training=True)
    
    # Pre-compute states only at chunk start positions (not fine-grained intervals)
    ...
    for chunk_start in chunk_starts:
        ...
        if chunk_start > current_pos:
            ...
            # Roll out this segment to get the state at chunk_start
            _, new_state = roll_out_func(inp_segment, current_state, return_diagnostics=False)
    ...
    return state_cache, chunk_ranges
...
def main(_):
    ...
    def run_step(sid: int, training=True, 
        ...
        # Pre-compute states for overlapping chunks (Phase B implementation)
        if flags.enable_state_precomputation:
            ...
            # Pre-compute states at stride intervals
            state_cache, _ = precompute_states_with_stride(
        ...
        for chunk_idx, (start, end) in enumerate(chunk_ranges):
            ...
            trg_np = trg_full[start:end, :]
            if aud.shape[0] < 2 or trg_np.shape[0] < 2:
                continue

            inp_kTN = tf.expand_dims(tf.convert_to_tensor(aud, dtype=tf.float32), 0)
            trg = tf.convert_to_tensor(trg_np, dtype=tf.float32)  # [time, features]
            trg = trg * flags.target_scaling
...
def _define_flags():
    ...
    f.DEFINE_integer("chunk_stride_ms", 50, "Stride between chunk starts in ms (smaller = more overlap)")
    ...
    f.DEFINE_integer("gradient_accumulation_steps", 5, "Number of chunks to accumulate gradients over before applying")

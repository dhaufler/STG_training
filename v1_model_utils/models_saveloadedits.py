import numpy as np
import tensorflow as tf
import os 
import pickle as pkl
# import subprocess
from . import other_v1_utils


# Define a custom gradient for the spike function.
# Diverse functions can be used to define the gradient.
# Here we provide variations of this functions depending on
# the gradient type and the precision of the input tensor.

def gauss_pseudo(v_scaled, sigma, amplitude):
    return tf.math.exp(-tf.square(v_scaled) / tf.square(sigma)) * amplitude

def pseudo_derivative(v_scaled, dampening_factor):
    return dampening_factor * tf.maximum(1 - tf.abs(v_scaled), 0)

# def slayer_pseudo(v_scaled, sigma, amplitude):
#     return tf.math.exp(-sigma * tf.abs(v_scaled)) * amplitude

# @tf.custom_gradient
# def spike_slayer(v_scaled, sigma, amplitude):
#     z_ = tf.greater(v_scaled, 0.0)
#     z_ = tf.cast(z_, tf.float32)

#     def grad(dy):
#         de_dz = dy
#         dz_dv_scaled = slayer_pseudo(v_scaled, sigma, amplitude)

#         de_dv_scaled = de_dz * dz_dv_scaled

#         return [de_dv_scaled, tf.zeros_like(sigma), tf.zeros_like(amplitude)]

#     return tf.identity(z_, name="spike_slayer"), grad

@tf.custom_gradient
def spike_gauss(v_scaled, sigma, amplitude):
    z_ = tf.greater(v_scaled, 0.0)
    z_ = tf.cast(z_, tf.float32)

    def grad(dy):
        de_dz = dy
        dz_dv_scaled = gauss_pseudo(v_scaled, sigma, amplitude)
        de_dv_scaled = de_dz * dz_dv_scaled

        return [de_dv_scaled, tf.zeros_like(sigma), tf.zeros_like(amplitude)]

    return tf.identity(z_, name="spike_gauss"), grad

@tf.custom_gradient
def spike_gauss_16(v_scaled, sigma, amplitude):
    z_ = tf.greater(v_scaled, 0.0)
    z_ = tf.cast(z_, tf.float16)

    def grad(dy):
        de_dz = dy
        dz_dv_scaled = gauss_pseudo(v_scaled, sigma, amplitude)

        de_dv_scaled = de_dz * dz_dv_scaled

        return [de_dv_scaled, tf.zeros_like(sigma), tf.zeros_like(amplitude)]

    return tf.identity(z_, name="spike_gauss"), grad


@tf.custom_gradient
def spike_gauss_b16(v_scaled, sigma, amplitude):
    z_ = tf.greater(v_scaled, 0.)
    z_ = tf.cast(z_, tf.bfloat16)

    def grad(dy):
        de_dz = dy
        dz_dv_scaled = gauss_pseudo(v_scaled, sigma, amplitude)

        de_dv_scaled = de_dz * dz_dv_scaled

        return [de_dv_scaled, tf.zeros_like(sigma), tf.zeros_like(amplitude)]

    return tf.identity(z_, name='spike_gauss'), grad

@tf.custom_gradient
def spike_function(v_scaled, dampening_factor):
    z_ = tf.greater(v_scaled, 0.0)
    z_ = tf.cast(z_, tf.float32)

    def grad(dy):
        de_dz = dy
        dz_dv_scaled = pseudo_derivative(v_scaled, dampening_factor)

        de_dv_scaled = de_dz * dz_dv_scaled

        return [de_dv_scaled, tf.zeros_like(dampening_factor)]

    return tf.identity(z_, name="spike_function"), grad

@tf.custom_gradient
def spike_function_16(v_scaled, dampening_factor):
    z_ = tf.greater(v_scaled, 0.0)
    z_ = tf.cast(z_, tf.float16)

    def grad(dy):
        de_dz = dy
        dz_dv_scaled = pseudo_derivative(v_scaled, dampening_factor)

        de_dv_scaled = de_dz * dz_dv_scaled

        return [de_dv_scaled, tf.zeros_like(dampening_factor)]

    return tf.identity(z_, name="spike_function"), grad

@tf.custom_gradient
def spike_function_b16(v_scaled, dampening_factor):
    z_ = tf.greater(v_scaled, 0.0)
    z_ = tf.cast(z_, tf.bfloat16)

    def grad(dy):
        de_dz = dy
        dz_dv_scaled = pseudo_derivative(v_scaled, dampening_factor)

        de_dv_scaled = de_dz * dz_dv_scaled

        return [de_dv_scaled, tf.zeros_like(dampening_factor)]

    return tf.identity(z_, name="spike_function"), grad

def exp_convolve(tensor, decay=0.8, reverse=False, initializer=None, axis=0):
    rank = len(tensor.get_shape())
    perm = np.arange(rank)
    perm[0], perm[axis] = perm[axis], perm[0]
    tensor = tf.transpose(tensor, perm)

    if initializer is None:
        initializer = tf.zeros_like(tensor[0])

    def scan_fun(_acc, _t):
        return _acc * decay + _t

    filtered = tf.scan(scan_fun, tensor, reverse=reverse,
                       initializer=initializer)

    filtered = tf.transpose(filtered, perm)
    return filtered

def make_pre_ind_table(indices, n_source_neurons=197613):
    """
    This function creates a table that maps presynaptic indices to 
    the indices of the recurrent_indices tensor using a RaggedTensor.
    This approach ensures that every presynaptic neuron, even those with no
    postsynaptic connections, has an entry in the RaggedTensor.
    """
    pre_inds = indices[:, 1]
    n_syn = pre_inds.shape[0]
    # Since pre_inds may not be sorted, we sort them along with synapse_indices
    sorted_pre_inds, sorted_synapse_indices = tf.math.top_k(-pre_inds, k=n_syn)
    # Count occurrences (out-degrees) for each presynaptic neuron using bincount
    sorted_pre_inds = tf.cast(-sorted_pre_inds, dtype=tf.int32)  # Undo the negation to get the sorted pre_inds and cast to reduce memory usage
    # Count occurrences (out-degrees) for each presynaptic neuron using bincount
    sorted_pre_inds = tf.cast(sorted_pre_inds, tf.int32)
    counts = tf.math.bincount(sorted_pre_inds, minlength=n_source_neurons)
    # Create row_splits that covers all presynaptic neurons (0 to n_source_neurons)
    row_splits = tf.concat([[0], tf.cumsum(counts)], axis=0)
    # Create the RaggedTensor with empty rows for missing neurons
    rt = tf.RaggedTensor.from_row_splits(sorted_synapse_indices, row_splits)

    return rt

def get_new_inds_table(indices, non_zero_cols, pre_ind_table):
    """Optimized function that prepares new sparse indices tensor."""
    # Gather the rows corresponding to the non_zero_cols
    selected_rows = tf.gather(pre_ind_table, non_zero_cols)
    # Flatten the selected rows to get all_inds
    all_inds = selected_rows.flat_values
    # Gather from indices using all_inds
    new_indices = tf.gather(indices, all_inds)

    return new_indices, all_inds


# class BackgroundNoiseLayer(tf.keras.layers.Layer):
#     """
#     This class calculates the input currents from the BKG noise by processing all timesteps at once."
#     For that reason is unfeasible if the user wants to train the LGN -> V1 weights.
#     Each call takes 0.03 seconds for 600 ms of simulation.

#     Returns:
#         _type_: input_currents (self.compute_dtype)
#     """
#     def __init__(self, indices, weights, dense_shape,  
#                  weights_factors, batch_size, seq_len,
#                  bkg_firing_rate=250, n_bkg_units=100, 
#                  **kwargs):
#         super().__init__(**kwargs)
#         self._bkg_weights = weights
#         self._bkg_indices = indices
#         self._dense_shape = dense_shape
#         self._bkg_input_weights_factors = weights_factors
#         self._batch_size = batch_size
#         self._seq_len = seq_len
#         self._n_syn_basis = weights_factors.shape[1]
#         # self._lr_scale = lr_scale
#         self._bkg_firing_rate = bkg_firing_rate
#         self._n_bkg_units = n_bkg_units

#     def calculate_bkg_i_in(self, inputs):
#         # This function performs the tensor multiplication to calculate the recurrent currents at each timestep
#         i_in = tf.TensorArray(dtype=self.compute_dtype, size=self._n_syn_basis)
#         for r_id in range(self._n_syn_basis):
#             weights_syn_receptors = self._bkg_weights * self._bkg_input_weights_factors[:, r_id]
#             sparse_w_in = tf.sparse.SparseTensor(
#                 self._bkg_indices,
#                 weights_syn_receptors, 
#                 self._dense_shape,
#             )
#             i_receptor = tf.sparse.sparse_dense_matmul(
#                                                         sparse_w_in,
#                                                         inputs,
#                                                         adjoint_b=True
#                                                     )
#             # Optionally cast the output back to float16
#             if i_receptor.dtype != self.compute_dtype:
#                 i_receptor = tf.cast(i_receptor, dtype=self.compute_dtype)

#             # Append i_receptor to the TensorArray
#             i_in = i_in.write(r_id, i_receptor)

#         # Stack the TensorArray into a single tensor
#         i_in = i_in.stack()

#         return i_in

#     @tf.function
#     def call(self, inp): # inp only provides the shape
#         # Generate the background spikes
#         seq_len = tf.shape(inp)[1]
#         rest_of_brain = tf.random.poisson(shape=(self._batch_size * seq_len, self._n_bkg_units), 
#                                         lam=self._bkg_firing_rate * .001, 
#                                         dtype=self.variable_dtype) # this implementation is slower
        
#         # rest_of_brain = tf.cast(tf.random.uniform(
#         #         (self._batch_size, seq_len, self._n_bkg_units)) < self._bkg_firing_rate * .001, 
#         #         tf.float32) # (1, 600, 100)
#         # rest_of_brain = tf.reshape(rest_of_brain, (self._batch_size * seq_len, self._n_bkg_units)) # (batch_size*sequence_length, input_dim)
       
#         # Create a TensorArray to save the results for every receptor type
#         noise_input = self.calculate_bkg_i_in(rest_of_brain) # (5, 65871, 600)

#         noise_input = tf.transpose(noise_input, (2, 1, 0)) # (600, 65871, 5) 
#         # Reshape properly the input current
#         noise_input = tf.reshape(noise_input, (self._batch_size, seq_len, -1)) # (1, 600, 250000) # (1, 3000, 333170)
#         # noise_input = tf.expand_dims(noise_input, axis=0) # (1, 600, 65871, 5)

#         return noise_input

@tf.keras.utils.register_keras_serializable(package='Custom')    
class BKGInputLayerCell(tf.keras.layers.Layer):
    def __init__(self, indices, weights, dense_shape, syn_ids, synaptic_basis_weights,
                 **kwargs):
        super().__init__(**kwargs)
        # Convert inputs to appropriate tensor types during initialization
        print("indices type:", type(indices))
        self._indices = tf.convert_to_tensor(indices, dtype=tf.int32)
        print("indices type:", type(indices))
        self._input_weights = tf.convert_to_tensor(weights, dtype=tf.float32)
        self._dense_shape = dense_shape
        self._input_syn_ids = tf.convert_to_tensor(syn_ids, dtype=tf.int32)
        self._synaptic_basis_weights = tf.convert_to_tensor(synaptic_basis_weights, dtype=tf.float32)
        
        # Precompute the synapses table - ensure it's also serializable
        self.pre_ind_table = make_pre_ind_table(indices, n_source_neurons=dense_shape[1])
        
        # tf.convert_to_tensor(
        #     make_pre_ind_table(indices, n_source_neurons=dense_shape[1]),
        #     dtype=tf.int32
        # )

    @property
    def state_size(self):
        # No states are maintained in this cell
        return []

    # def build(self, input_shape):
    #     # If you have any trainable variables, initialize them here
    #     pass

    # @tf.function
    def call(self, inputs_t, states):
        # inputs_t: Shape [batch_size, input_dim]
        # make sure inputs_t dtype is float64
        #inputs_t = tf.cast(inputs_t, dtype=tf.float64)
        batch_size = tf.shape(inputs_t)[0]
        # Compute the input current for the timestep
        non_zero_cols = tf.where(inputs_t > 0)[:, 1]
        new_indices, inds = get_new_inds_table(self._indices, non_zero_cols, self.pre_ind_table)
        # Sort the segment IDs and corresponding data
        # sorted_indices = tf.argsort(new_indices[:, 0])
        # sorted_segment_ids = tf.gather(new_indices[:, 0], sorted_indices)
        # sorted_inds = tf.gather(inds, sorted_indices)

        # Get the number of presynaptic spikes
        n_pre_spikes = tf.cast(tf.gather(inputs_t[0, :], new_indices[:, 1]), dtype=self.variable_dtype)
        
        # Get the weights for each active synapse
        sorted_syn_ids = tf.gather(self._input_syn_ids, inds)
        sorted_weights = tf.expand_dims(tf.gather(self._input_weights, inds, axis=0) * n_pre_spikes, axis=1)
        sorted_data = sorted_weights * tf.gather(self._synaptic_basis_weights, sorted_syn_ids, axis=0)
        # sorted_data = tf.gather(self._input_weights, inds, axis=0) * tf.gather(self._input_weights_factors, inds, axis=0)
        # Calculate the total LGN input current received by each neuron
        i_in = tf.math.unsorted_segment_sum(
            sorted_data,
            new_indices[:, 0],
            num_segments=self._dense_shape[0]
        )
        # Optionally cast the output back to float16
        if i_in.dtype != self.compute_dtype:
            i_in = tf.cast(i_in, dtype=self.compute_dtype)

        # Add batch dimension
        # i_in = tf.expand_dims(i_in, axis=0)  # Shape: [1, n_post_neurons, n_syn_basis]
        i_in = tf.reshape(i_in, [batch_size, -1])
        # Since no states are maintained, return empty state
        return i_in, []
    
    def get_config(self):
        base_config = super().get_config()
        return {
            **base_config,
            'indices': self._indices.numpy().tolist(),
            'weights': self._input_weights.numpy().tolist(),
            'dense_shape': self._dense_shape,
            'syn_ids': self._input_syn_ids.numpy().tolist(),
            'synaptic_basis_weights': self._synaptic_basis_weights.numpy().tolist()
        }
    
    @classmethod
    def from_config(cls, config):
        config['indices'] = np.array(config['indices'])
        config['weights'] = np.array(config['weights'])
        config['dense_shape'] = tuple(config['dense_shape'])  # Convert to tuple
        config['syn_ids'] = np.array(config['syn_ids'])
        config['synaptic_basis_weights'] = np.array(config['synaptic_basis_weights'])
        return cls(**config)

@tf.keras.utils.register_keras_serializable(package='Custom')
class BKGInputLayer(tf.keras.layers.Layer):
    """
    Calculates input currents from the LGN by processing one timestep at a time using a custom RNN cell.
    """
    def __init__(self, indices, BKG_weights, dense_shape, syn_ids, synaptic_basis_weights,
                 batch_size, bkg_firing_rate=250, n_bkg_units=100,
                 **kwargs):
        super().__init__(**kwargs)
        # Convert inputs to appropriate tensor types during initialization
        self.indices = tf.convert_to_tensor(indices, dtype=tf.int32)
        self.BKG_weights = tf.Variable(BKG_weights, dtype=tf.float32, trainable=False)
        self.dense_shape = dense_shape
        self.syn_ids = tf.convert_to_tensor(syn_ids, dtype=tf.int32)
        self.synaptic_basis_weights = tf.convert_to_tensor(synaptic_basis_weights, dtype=tf.float32)
        self.batch_size = batch_size
        self.bkg_firing_rate = bkg_firing_rate
        self.n_bkg_units = n_bkg_units
        
        self.input_cell = BKGInputLayerCell(
            self.indices, self.BKG_weights, dense_shape, self.syn_ids, 
            self.synaptic_basis_weights, **kwargs
        )
        # Create the input RNN layer with the custom cell to recursively process all the inputs by timesteps
        self.input_rnn = tf.keras.layers.RNN(self.input_cell, return_sequences=True, return_state=False, name='noise_rsnn')

    @tf.function
    def call(self, inputs, **kwargs):
        seq_len = tf.shape(inputs)[1]
        rest_of_brain = tf.random.poisson(shape=(self.batch_size, seq_len, self.n_bkg_units), 
                                    lam=self.bkg_firing_rate * .001, 
                                    dtype=self.variable_dtype) # this implementation is slower
        # inputs: Shape [batch_size, seq_len, input_dim]
        input_current = self.input_rnn(rest_of_brain, **kwargs)  # Outputs: [batch_size, seq_len, n_postsynaptic_neurons]

        return input_current
    
    def get_config(self):
        base_config = super().get_config()
        # preserves the model's configuration from parent layer class
        # such as layer name, trainable status, etc.
        #Convert tensorflow variables/tensors to numpy arrays before serialization
        config = {
            'indices': self.indices.numpy().tolist(),
            'BKG_weights': self.BKG_weights.numpy().tolist(),
            'dense_shape': self.dense_shape,
            'syn_ids': self.syn_ids.numpy().tolist(),
            'synaptic_basis_weights': self.synaptic_basis_weights.numpy().tolist(),
            'batch_size': self.batch_size,
            'bkg_firing_rate': self.bkg_firing_rate,
            'n_bkg_units': self.n_bkg_units
        }
        return {**base_config, **config}
    
    # reconstructs layer from config dict created by get_config
    @classmethod
    def from_config(cls, config):
        # Reconstruct tensors with proper dtypes before instantiating
        config = config.copy()  # Make a copy to avoid modifying the original
        config['indices'] = tf.convert_to_tensor(
            config['indices'], dtype=tf.int32)
        config['BKG_weights'] = tf.Variable(
            config['BKG_weights'], dtype=tf.float32, trainable=False)
        config['dense_shape'] = tuple(config['dense_shape'])  # Convert to tuple
        config['syn_ids'] = tf.convert_to_tensor(
            config['syn_ids'], dtype=tf.int32)
        config['synaptic_basis_weights'] = tf.convert_to_tensor(
            config['synaptic_basis_weights'], dtype=tf.float32)
        
        print("BKG_weights type:", type(config['BKG_weights']))
        print("BKG_weights shape:", tf.shape(config['BKG_weights']))
        print("BKG_weights dtype:", config['BKG_weights'].dtype)
        return cls(**config)
 
@tf.keras.utils.register_keras_serializable(package='Custom')   
class LGNInputLayerCell(tf.keras.layers.Layer):
    def __init__(self, indices, weights, dense_shape, syn_ids, synaptic_basis_weights,
                 **kwargs):
        super().__init__(**kwargs)
        
        self._indices = tf.convert_to_tensor(indices)
        self._input_weights = tf.convert_to_tensor(weights)
        self._input_syn_ids = tf.convert_to_tensor(syn_ids)
        self._synaptic_basis_weights = tf.convert_to_tensor(synaptic_basis_weights)
        self._dense_shape = dense_shape
        self.pre_ind_table = make_pre_ind_table(indices, n_source_neurons=dense_shape[1])

    @property
    def state_size(self):
        # No states are maintained in this cell
        return []

    # def build(self, input_shape):
    #     # If you have any trainable variables, initialize them here
    #     pass

    # @tf.function
    def call(self, inputs_t, states):
        # inputs_t: Shape [batch_size, input_dim]
        batch_size = tf.shape(inputs_t)[0]
        # Compute the input current for the timestep
        non_zero_cols = tf.where(inputs_t > 0)[:, 1]
        new_indices, inds = get_new_inds_table(self._indices, non_zero_cols, self.pre_ind_table)
        # Sort the segment IDs and corresponding data
        # sorted_indices = tf.argsort(new_indices[:, 0])
        # sorted_segment_ids = tf.gather(new_indices[:, 0], sorted_indices)
        # sorted_inds = tf.gather(inds, sorted_indices)
        
        # Get the weights for each active synapse
        sorted_syn_ids = tf.gather(self._input_syn_ids, inds)
        sorted_weights = tf.expand_dims(tf.gather(self._input_weights, inds, axis=0), axis=1)
        sorted_data = sorted_weights * tf.gather(self._synaptic_basis_weights, sorted_syn_ids, axis=0)
        # sorted_data = tf.gather(self._input_weights, inds, axis=0) * tf.gather(self._input_weights_factors, inds, axis=0)
        # Calculate the total LGN input current received by each neuron
        i_in = tf.math.unsorted_segment_sum(
            sorted_data,
            new_indices[:, 0],
            num_segments=self._dense_shape[0]
        )
        # Optionally cast the output back to float16
        if i_in.dtype != self.compute_dtype:
            i_in = tf.cast(i_in, dtype=self.compute_dtype)

        # Add batch dimension
        # i_in = tf.expand_dims(i_in, axis=0)  # Shape: [1, n_post_neurons, n_syn_basis]
        i_in = tf.reshape(i_in, [batch_size, -1])
        # Since no states are maintained, return empty state
        return i_in, []
    
    def get_config(self):
        config = super().get_config()
        config.update({
            'indices': self._indices.numpy().tolist(),
            'weights': self._input_weights.numpy().tolist(),
            'dense_shape': self._dense_shape,
            'syn_ids': self._input_syn_ids.numpy().tolist(),
            'synaptic_basis_weights': self._synaptic_basis_weights.numpy().tolist(),
        })
        return config

    @classmethod
    def from_config(cls, config):
        config['indices'] = np.array(config['indices'])
        config['weights'] = np.array(config['weights'])
        config['dense_shape'] = tuple(config['dense_shape'])  # Convert to tuple
        config['syn_ids'] = np.array(config['syn_ids'])
        config['synaptic_basis_weights'] = np.array(config['synaptic_basis_weights'])
        return cls(**config)
    
    
@tf.keras.utils.register_keras_serializable(package='Custom')
class LGNInputLayer(tf.keras.layers.Layer):
    """
    Calculates input currents from the LGN by processing one timestep at a time using a custom RNN cell.
    """
    def __init__(self, indices, LGN_weights, dense_shape, syn_ids, synaptic_basis_weights,
                 **kwargs):
        super().__init__(**kwargs)
        
        # Convert inputs to tensors if they aren't already
        self.indices = tf.convert_to_tensor(indices)
        self.LGN_weights = tf.convert_to_tensor(LGN_weights)
        self.dense_shape = dense_shape
        self.syn_ids = tf.convert_to_tensor(syn_ids)
        self.synaptic_basis_weights = tf.convert_to_tensor(synaptic_basis_weights)
        
        self.input_cell = LGNInputLayerCell(
            indices, LGN_weights, dense_shape, syn_ids, synaptic_basis_weights,
            **kwargs
        )
        # Create the input RNN layer with the custom cell to recursively process all the inputs by timesteps
        self.input_rnn = tf.keras.layers.RNN(self.input_cell, return_sequences=True, return_state=False, name='lgn_rsnn')

    @tf.function
    def call(self, inputs, **kwargs):
        # inputs: Shape [batch_size, seq_len, input_dim]
        input_current = self.input_rnn(inputs, **kwargs)  # Outputs: [batch_size, seq_len, n_postsynaptic_neurons]

        return input_current
    
    def _serialize_tensor(self, tensor):
        """Safely convert tensor to serializable format"""
        try:
            if isinstance(tensor, tf.Variable):
                return tensor.numpy().tolist()
            elif isinstance(tensor, tf.Tensor):
                return tensor.numpy().tolist()
            elif isinstance(tensor, np.ndarray):
                return tensor.tolist()
            elif isinstance(tensor, list):
                return tensor
            else:
                raise ValueError(f"Unsupported type for serialization: {type(tensor)}")
        except Exception as e:
            raise ValueError(f"Error serializing tensor: {str(e)}")
    
    def get_config(self):
        base_config = super().get_config()
        try:
            config = {
                'indices': self._serialize_tensor(self.indices),
                'LGN_weights': self._serialize_tensor(self.LGN_weights),
                'dense_shape': self.dense_shape,
                'syn_ids': self._serialize_tensor(self.syn_ids),
                'synaptic_basis_weights': self._serialize_tensor(self.synaptic_basis_weights),
            }
            return {**base_config, **config}
        except Exception as e:
            raise ValueError(f"Error in get_config: {str(e)}")
    
    # reconstructs layer from config dict created by get_config
    @classmethod
    def from_config(cls, config):
        try:
            # Convert lists back to numpy arrays
            config['indices'] = np.array(config['indices'])
            config['LGN_weights'] = np.array(config['LGN_weights'])
            config['dense_shape'] = tuple(config['dense_shape'])  # Convert to tuple
            config['syn_ids'] = np.array(config['syn_ids'])
            config['synaptic_basis_weights'] = np.array(config['synaptic_basis_weights'])
            return cls(**config)
        except Exception as e:
            raise ValueError(f"Error in from_config: {str(e)}")
    


class SignedConstraint(tf.keras.constraints.Constraint):
    def __init__(self, positive):
        self._positive = positive

    def __call__(self, w):
        sign_corrected_w = tf.where(
            self._positive, tf.nn.relu(w), -tf.nn.relu(-w))
        return sign_corrected_w
    
    def get_config(self):
        return {
            'positive': self._positive,
        }


class SparseSignedConstraint(tf.keras.constraints.Constraint):
    def __init__(self, mask, positive):
        self._mask = mask
        self._positive = positive

    def __call__(self, w):
        sign_corrected_w = tf.where(
            self._positive, tf.nn.relu(w), -tf.nn.relu(-w))
        return tf.where(self._mask, sign_corrected_w, tf.zeros_like(sign_corrected_w))
    
    def get_config(self):
        return {
            'mask': self._mask,
            'positive': self._positive,
        }


class ClipConstraint(tf.keras.constraints.Constraint):
    def __init__(self, lower_limit, upper_limit):
        self._lower_limit = lower_limit
        self._upper_limit = upper_limit

    def __call__(self, w):
        return tf.clip_by_value(w, self._lower_limit, self._upper_limit)
    
    def get_config(self):
        return {
            'lower_limit': self._lower_limit,
            'upper_limit': self._upper_limit,
        }
    
@tf.keras.utils.register_keras_serializable(package='Custom')
class V1Column(tf.keras.layers.Layer):
    def __init__(
        self,
        network,
        lgn_input,
        bkg_input,
        data_dir,
        dt=1.0,
        gauss_std=0.5,
        dampening_factor=0.3,
        recurrent_dampening_factor=0.4,
        input_weight_scale=1.0,
        recurrent_weight_scale=1.0,
        lr_scale=1.0,
        spike_gradient=False,
        max_delay=5,
        n_syn_basis = 5,
        pseudo_gauss=False,
        train_recurrent=True,
        train_recurrent_per_type=True,
        train_input=False,
        train_noise=True,
        hard_reset=True,
        **kwargs    # Catch any other Keras parameters
    ):
        
        super().__init__(**kwargs)
        
        # Store all initialization parameters as instance variables
        self._network = network
        self._lgn_input = lgn_input
        self._bkg_input = bkg_input
        self._data_dir = data_dir
        self._dt = dt
        self._gauss_std = gauss_std
        self._dampening_factor = dampening_factor
        self._recurrent_dampening_factor = recurrent_dampening_factor
        self._input_weight_scale = input_weight_scale
        self._recurrent_weight_scale = recurrent_weight_scale
        self._lr_scale = lr_scale
        self._spike_gradient = spike_gradient
        self._max_delay = max_delay
        self._n_syn_basis = n_syn_basis
        self._pseudo_gauss = pseudo_gauss
        self._train_recurrent = train_recurrent
        self._train_recurrent_per_type = train_recurrent_per_type
        self._train_input = train_input
        self._train_noise = train_noise
        self._hard_reset = hard_reset
        
        self._recurrent_dampening = recurrent_dampening_factor
        
        # print data dtypes of all inputs to V1Column
        print('CHECK DATA TYPES')
        print("Data types of inputs to V1Column:")
        print(f"network: {type(network)}")
        print(f"lgn_input: {type(lgn_input)}")
        print(f"bkg_input: {type(bkg_input)}")
        print(f"data_dir: {type(data_dir)}")
        print(f"dt: {type(dt)}")
        print(f"gauss_std: {type(gauss_std)}")
        print(f"dampening_factor: {type(dampening_factor)}")
        print(f"recurrent_dampening_factor: {type(recurrent_dampening_factor)}")
        print(f"input_weight_scale: {type(input_weight_scale)}")
        print(f"recurrent_weight_scale: {type(recurrent_weight_scale)}")
        print(f"lr_scale: {type(lr_scale)}")
        print(f"spike_gradient: {type(spike_gradient)}")
        print(f"max_delay: {type(max_delay)}")
        print(f"n_syn_basis: {type(n_syn_basis)}")
        print(f"pseudo_gauss: {type(pseudo_gauss)}")
        print(f"train_recurrent: {type(train_recurrent)}")
        print(f"train_recurrent_per_type: {type(train_recurrent_per_type)}")
        print(f"train_input: {type(train_input)}")
        print(f"train_noise: {type(train_noise)}")
        print(f"hard_reset: {type(hard_reset)}")
                
        params = dict(network["node_params"])
        # Ensure that all parameters are numpy arrays
        for key, value in params.items():
            if not isinstance(value, np.ndarray):
                params[key] = np.array(value)
        
        # Store the neuron parameters    
        self.V_th = params["V_th"]
        self.E_L = params["E_L"]
        self.V_reset = params["V_reset"]
        self.asc_amps = params["asc_amps"]
        self.g = params["g"]
        self.k = params["k"]
        self.t_ref = params["t_ref"]
        self.C_m = params["C_m"]
        self.node_type_ids = network["node_type_ids"]
        
        
        # Rescale the voltages to have them near 0, as we wanted the effective step size
        # for the weights to be normalized when learning (weights are scaled similarly)
        voltage_scale = params["V_th"] - params["E_L"]
        voltage_offset = params["E_L"]
        params["V_th"] = (params["V_th"] - voltage_offset) / voltage_scale
        params["E_L"] = (params["E_L"] - voltage_offset) / voltage_scale
        params["V_reset"] = (params["V_reset"] - voltage_offset) / voltage_scale
        params["asc_amps"] = (params["asc_amps"] / voltage_scale[..., None])  # params['asc_amps'] has shape (111, 2)

        self.n_neurons = int(network["n_nodes"])

        # Determine the membrane time decay constant
        tau = params["C_m"] / params["g"]
        membrane_decay = np.exp(-dt / tau)
        current_factor = 1 / params["C_m"] * (1 - membrane_decay) * tau

        # Determine the synaptic dynamic parameters for each of the 5 basis receptors.
        path='synaptic_data/tau_basis.npy' # [0.7579732  1.33243834 2.34228851 4.11750046 7.23813909]
        tau_syns = np.load(path)
        syn_decay = np.exp(-dt / tau_syns)
        syn_decay = tf.constant(syn_decay, dtype=self.compute_dtype)
        syn_decay = tf.tile(syn_decay, [self.n_neurons])
        self.syn_decay = tf.expand_dims(syn_decay, axis=0) # expand the dimension for processing different receptor types
        psc_initial = np.e / tau_syns
        psc_initial = tf.constant(psc_initial, dtype=self.compute_dtype)
        psc_initial = tf.tile(psc_initial, [self.n_neurons])
        self.psc_initial = tf.expand_dims(psc_initial, axis=0) # expand the dimension for processing different receptor types

        # Find the maximum delay in the network
        self.max_delay = int(np.round(np.min([np.max(network["synapses"]["delays"]), max_delay])))
        
        def _gather(prop):
            return tf.gather(prop, self.node_type_ids)
    
        def _f(_v, trainable=False):
            return tf.Variable(tf.cast(_gather(_v), self.compute_dtype), trainable=trainable)

        def inv_sigmoid(_x):
            return tf.math.log(_x / (1 - _x))

        def custom_val(_v, trainable=False):
            _v = tf.Variable(
                tf.cast(inv_sigmoid(_gather(_v)), self.compute_dtype),
                trainable=trainable,
            )
            def _g():
                return tf.nn.sigmoid(_v.read_value())

            return _v, _g

        # Gather the neuron parameters for every neuron
        self.t_ref = _f(params["t_ref"])  # refractory time
        self.v_reset = _f(params["V_reset"])
        self.asc_amps = _f(params["asc_amps"], trainable=False)
        _k = tf.cast(params['k'], self.compute_dtype)
        # inverse sigmoid of the adaptation rate constant (1/ms)
        param_k, param_k_read = custom_val(_k, trainable=False)
        k = param_k_read()
        self.exp_dt_k = tf.exp(-self._dt * k)

        self.v_th = _f(params["V_th"])
        self.v_gap = self.v_reset - self.v_th
        e_l = _f(params["E_L"])
        self.normalizer = self.v_th - e_l
        param_g = _f(params["g"])
        self.gathered_g = param_g * e_l

        self.decay = _f(membrane_decay)
        self.current_factor = _f(current_factor)
        self.voltage_scale = _f(voltage_scale)
        self.voltage_offset = _f(voltage_offset)

        # Find the synaptic basis representation for each synaptic type
        path = os.path.join(data_dir, 'syn_id_to_syn_weights_dict.pkl')
        with open(path, "rb") as f:
            syn_id_to_syn_weights_dict = pkl.load(f)
        synaptic_basis_weights = np.array(list(syn_id_to_syn_weights_dict.values()))
        self.synaptic_basis_weights = tf.constant(synaptic_basis_weights, dtype=self.variable_dtype)

        ### Network recurrent connectivity ###
        self.indices = np.array(network["synapses"]["indices"])
        self.recurrent_weights = np.array(network["synapses"]["weights"])
        self.dense_shape = np.array(network["synapses"]["dense_shape"])
        syn_ids = np.array(network["synapses"]["syn_ids"])
        self.delays = np.array(network["synapses"]["delays"])

        # Scale down the recurrent weights
        weights = (self.recurrent_weights/voltage_scale[self.node_type_ids[self.indices[:, 0]]])      
        dense_shape = self.dense_shape
        # Use the maximum delay to clip the synaptic delays
        delays = np.round(np.clip(self.delays, dt, self.max_delay)/dt).astype(np.int32)
        # Introduce the delays in the presynaptic neuron indices
        indices = self.indices
        indices[:, 1] = indices[:, 1] + self.n_neurons * (delays - 1)

        self.recurrent_dense_shape = dense_shape[0], self.max_delay * dense_shape[1] 
        #the first column (presynaptic neuron) has size n_neurons and the second column (postsynaptic neuron) has size max_delay*n_neurons

        # Define the Tensorflow variables
        # self.recurrent_indices = tf.Variable(indices, dtype=tf.int64, trainable=False)
        self.recurrent_indices = tf.Variable(indices, dtype=tf.int32, trainable=False)
        self.pre_ind_table = make_pre_ind_table(indices, n_source_neurons=self.recurrent_dense_shape[1])

        # add dimension for the weights factors - TensorShape([23525415, 1])
        # weights = tf.expand_dims(weights, axis=1) 
        # Set the sign of the connections (exc or inh)
        recurrent_weight_positive = tf.Variable(
            weights >= 0.0, name="recurrent_weights_sign", trainable=False)

        # if training the recurrent connection per type, turn off recurrent training
        # of individual connections
        if train_recurrent:
            if train_recurrent_per_type:
                individual_training = False
                per_type_training = True
            else:
                individual_training = True
                per_type_training = False
        else:
            individual_training = False
            per_type_training = False

        # Scale the weights
        self.recurrent_weight_values = tf.Variable(
            weights * recurrent_weight_scale / lr_scale, 
            name="sparse_recurrent_weights",
            constraint=SignedConstraint(recurrent_weight_positive),
            trainable=individual_training,
            dtype=self.variable_dtype
        ) # shape = (n_synapses,)

        # prepare per_type variable, if required
        if per_type_training:
            self.per_type_training = True
            self.connection_type_ids = other_v1_utils.connection_type_ids(network)
            max_id = np.max(self.connection_type_ids) + 1
            # prepare a variable and gather with type ids.
            self.recurrent_per_type_weight_values = tf.Variable(
                tf.ones(max_id),
                name="recurrent_per_type_weights",
                constraint=ClipConstraint(0.2, 5.0),
                trainable=True,
                dtype=self.variable_dtype
            ) # shape = (n_connection_types (21 * 21))
            # multiply this to the weights (this needs to be done in the loop)
        else:
            self.per_type_training = False
            
        # self.syn_ids = tf.constant(syn_ids, dtype=tf.int64)
        self.syn_ids = tf.constant(syn_ids, dtype=tf.int32)
        # self.recurrent_weights_factors = tf.gather(self.synaptic_basis_weights, self.syn_ids, axis=0) # TensorShape([23525415, 5])
        print(f"    > # Recurrent synapses: {len(indices)}")

        del indices, weights, dense_shape, delays, syn_ids

        ### LGN input connectivity ###
        self.lgn_input_dense_shape = (self.n_neurons, lgn_input["n_inputs"],)
        input_indices = np.array(lgn_input["indices"])
        self._input_indices = np.array(lgn_input["indices"])
        self.input_weights = np.array(lgn_input["weights"])
        self._input_syn_ids = np.array(lgn_input["syn_ids"])
        input_syn_ids = np.array(lgn_input["syn_ids"])
        self.input_delays = np.array(lgn_input["delays"])

        # Scale down the input weights
        input_weights = (self.input_weights/ voltage_scale[self.node_type_ids[input_indices[:, 0]]])
        input_delays = np.round(np.clip(self.input_delays, dt, self.max_delay)/dt).astype(np.int32)
        # Introduce the delays in the postsynaptic neuron indices
        # input_indices[:, 1] = input_indices[:, 1] + self.n_neurons * (input_delays - 1)
        # self.input_indices = tf.Variable(input_indices, trainable=False, dtype=tf.int64)
        self.input_indices = tf.Variable(input_indices, trainable=False, dtype=tf.int32)

        # Define the Tensorflow variables
        # input_weights = tf.expand_dims(input_weights, axis=1) # add dimension for the weights factors - TensorShape([23525415, 1])

        input_weight_positive = tf.Variable(
            input_weights >= 0.0, name="input_weights_sign", trainable=False)
        
        self.input_weight_values = tf.Variable(
            input_weights * input_weight_scale / lr_scale,
            name="sparse_input_weights",
            constraint=SignedConstraint(input_weight_positive),
            trainable=train_input,
            dtype=self.variable_dtype
        )

        # self.input_syn_ids = tf.constant(input_syn_ids, dtype=tf.int64)
        self.input_syn_ids = tf.constant(input_syn_ids, dtype=tf.int32)
        # self.input_weights_factors = tf.gather(self.synaptic_basis_weights, input_syn_ids, axis=0)

        print(f"    > # LGN input synapses {len(input_indices)}")
        del input_indices, input_weights, input_syn_ids, input_delays

        ### BKG input connectivity ###
        self.bkg_input_dense_shape = (self.n_neurons, bkg_input["n_inputs"],)
        bkg_input_indices = np.array(bkg_input['indices'])
        self._bkg_input_indices = np.array(bkg_input['indices'])
        self._bkg_input_weights = np.array(bkg_input['weights'])
        self._bkg_input_syn_ids = np.array(bkg_input['syn_ids'])
        bkg_input_syn_ids = np.array(bkg_input['syn_ids'])
        self.bkg_input_delays = np.array(bkg_input['delays'])

        bkg_input_weights = (self._bkg_input_weights/voltage_scale[self.node_type_ids[bkg_input_indices[:, 0]]])
        bkg_input_delays = np.round(np.clip(self.bkg_input_delays, dt, self.max_delay)/dt).astype(np.int32)
        # Introduce the delays in the postsynaptic neuron indices
        # bkg_input_indices[:, 1] = bkg_input_indices[:, 1] + self.n_neurons * (bkg_input_delays - 1)
        # self.bkg_input_indices = tf.Variable(bkg_input_indices, trainable=False, dtype=tf.int64)
        self.bkg_input_indices = tf.Variable(bkg_input_indices, trainable=False, dtype=tf.int32)

        # Define Tensorflow variables
        bkg_input_weight_positive = tf.Variable(
            bkg_input_weights >= 0.0, name="bkg_input_weights_sign", trainable=False)
        self.bkg_input_weights = tf.Variable(
            bkg_input_weights * input_weight_scale / lr_scale, 
            name="rest_of_brain_weights", 
            constraint=SignedConstraint(bkg_input_weight_positive),
            trainable=train_noise,
            dtype=self.variable_dtype
        )

        #self.bkg_input_syn_ids = tf.constant(bkg_input_syn_ids, dtype=tf.int64)
        self.bkg_input_syn_ids = tf.constant(bkg_input_syn_ids, dtype=tf.int32)
        # self.bkg_input_weights_factors = tf.gather(self.synaptic_basis_weights, bkg_input_syn_ids, axis=0)

        print(f"    > # BKG input synapses {len(bkg_input_indices)}")
        del bkg_input_indices, bkg_input_weights, bkg_input_syn_ids, bkg_input_delays

    def calculate_i_rec(self, rec_z_buf):
        print('CALCULATE_I_REC START')
        # calculates the recurrent currents received by each neuron at each timestep
        # This is a new faster implementation that uses the pre_ind_table as a raggedTensor and exploits
        # the sparseness of the rec_z_buf.
        # it identifies the non_zero rows of rec_z_buf and only computes the
        # contributions for those rows.        
        non_zero_cols = tf.where(rec_z_buf > 0)[:, 1]
        new_indices, inds = get_new_inds_table(self.recurrent_indices, non_zero_cols, self.pre_ind_table)
        # # Sort the segment IDs and corresponding data
        # sorted_indices = tf.argsort(new_indices[:, 0])
        # sorted_segment_ids = tf.gather(new_indices[:, 0], sorted_indices)
        # sorted_inds = tf.gather(inds, sorted_indices)
        # Get the weights for each active synapse
        sorted_syn_ids = tf.gather(self.syn_ids, inds)
        sorted_weights = tf.expand_dims(tf.gather(self.recurrent_weight_values, inds, axis=0), axis=1)
        sorted_data = sorted_weights * tf.gather(self.synaptic_basis_weights, sorted_syn_ids, axis=0)
        # sorted_data = tf.gather(self.recurrent_weight_values, inds, axis=0) * tf.gather(self.recurrent_weights_factors, inds, axis=0)
        if self.per_type_training:
            per_type_weights = tf.expand_dims(tf.gather(self.recurrent_per_type_weight_values, 
                                                        tf.gather(self.connection_type_ids, inds)), axis=1)
            sorted_data = sorted_data * per_type_weights
        # Calculate the total recurrent current received by each neuron
        i_rec = tf.math.unsorted_segment_sum(
            sorted_data,
            new_indices[:, 0],
            num_segments=self.recurrent_dense_shape[0]
        )

        if i_rec.dtype != self.compute_dtype:
            i_rec = tf.cast(i_rec, dtype=self.compute_dtype)
        # Add batch dimension
        # i_rec = tf.expand_dims(i_rec, axis=0)
        i_rec = tf.reshape(i_rec, [1, -1])
        print('CALCULATE_I_REC DONE')
        return i_rec

              
    def update_psc(self, psc, psc_rise, rec_inputs):
        print('UPDATE_PSC START')
        new_psc_rise = psc_rise * self.syn_decay + rec_inputs * self.psc_initial
        new_psc = psc * self.syn_decay + self._dt * self.syn_decay * psc_rise
        print('UPDATE_PSC DONE')
        return new_psc, new_psc_rise

    @property
    def state_size(self):
        print('STATE_SIZE START')
        # Define the state size of the network
        state_size = (
            self.n_neurons * self.max_delay,  # z buffer
            self.n_neurons,  # v
            self.n_neurons,  # r
            self.n_neurons * 2,  # asc
            self.n_neurons * self._n_syn_basis,  # psc rise
            self.n_neurons * self._n_syn_basis,  # psc
        )
        print('STATE_SIZE DONE')
        return state_size
    
    def zero_state(self, batch_size, dtype=tf.float32):
        print('ZERO_STATE START')
        # The neurons membrane voltage start the simulation at their reset value
        z0_buf = tf.zeros((batch_size, self.n_neurons * self.max_delay), dtype)
        v0 = tf.ones((batch_size, self.n_neurons), dtype) * tf.cast(
            self.v_th * 0.0 + 1.0 * self.v_reset, dtype)
        r0 = tf.zeros((batch_size, self.n_neurons), dtype)
        asc = tf.zeros((batch_size, self.n_neurons * 2), dtype)
        psc_rise0 = tf.zeros((batch_size, self.n_neurons * self._n_syn_basis), dtype)
        psc0 = tf.zeros((batch_size, self.n_neurons * self._n_syn_basis), dtype)
        print('ZERO_STATE DONE')
        return z0_buf, v0, r0, asc, psc_rise0, psc0

    # @tf.function
    def call(self, inputs, state, constants=None):
        print('CALL START')
        # Get all the model inputs
        external_current = inputs[:, :self.n_neurons*self._n_syn_basis] # external inputs shape (1, 399804)
        bkg_noise = inputs[:, self.n_neurons*self._n_syn_basis:-self.n_neurons]
        state_input = inputs[:, -self.n_neurons:] # dummy zeros
        batch_size = tf.shape(bkg_noise)[0]

        if self._spike_gradient:
            state_input = tf.zeros((1,), dtype=self.compute_dtype)
        else:
            state_input = tf.zeros((4,), dtype=self.compute_dtype)
                
        # Extract the network variables from the state
        z_buf, v, r, asc, psc_rise, psc = state
        # Get previous spikes
        prev_z = z_buf[:, :self.n_neurons]  # Shape: [batch_size, n_neurons]
        # Define the spikes buffer
        dampened_z_buf = z_buf * self._recurrent_dampening  # dampened version of z_buf # no entiendo muy bien la utilidad de esto
        # Now we use tf.stop_gradient to prevent the term (z_buf - dampened_z_buf) to be trained
        rec_z_buf = (tf.stop_gradient(z_buf - dampened_z_buf) + dampened_z_buf)  
        # Calculate the recurrent postsynaptic currents
        i_rec = self.calculate_i_rec(rec_z_buf)
        # Add all the current sources
        rec_inputs = i_rec + external_current + bkg_noise

        # Scale with the learning rate
        rec_inputs = rec_inputs * self._lr_scale
        
        if constants is not None and not self._spike_gradient:
            rec_inputs = rec_inputs + state_input * self._lr_scale

        # Calculate the new psc variables
        new_psc, new_psc_rise = self.update_psc(psc, psc_rise, rec_inputs)        
        # Calculate the ASC
        asc = tf.reshape(asc, (batch_size, self.n_neurons, 2))
        new_asc = self.exp_dt_k * asc + tf.expand_dims(prev_z, axis=-1) * self.asc_amps
        new_asc = tf.reshape(new_asc, (batch_size, self.n_neurons * 2))
        # Calculate the postsynaptic current 
        input_current = tf.reshape(psc, (batch_size, self.n_neurons, self._n_syn_basis))
        input_current = tf.reduce_sum(input_current, -1) # sum over receptors
        if constants is not None and self._spike_gradient:
            input_current += state_input

        # Add all the postsynaptic current sources
        c1 = input_current + tf.reduce_sum(asc, axis=-1) + self.gathered_g

        # Calculate the new voltage values
        decayed_v = self.decay * v
        reset_current = prev_z * self.v_gap
        new_v = decayed_v + self.current_factor * c1 + reset_current

        # Update the voltage according to the LIF equation and the refractory period
        # New r is a variable that accounts for the refractory period in which a neuron cannot spike
        new_r = tf.nn.relu(r + prev_z * self.t_ref - self._dt)  # =max(r + prev_z * self.t_ref - self._dt, 0)
        if self._hard_reset:
            # Here we keep the voltage at the reset value during the refractory period
            new_v = tf.where(new_r > 0.0, self.v_reset, new_v)
            # Here we make a hard reset and let the voltage freely evolve but we do not let the
            # neuron spike during the refractory period

        # Generate the network spikes
        v_sc = (new_v - self.v_th) / self.normalizer
        if self._pseudo_gauss:
            if self.compute_dtype == tf.bfloat16:
                new_z = spike_gauss_b16(v_sc, self._gauss_std, self._dampening_factor)
            elif self.compute_dtype == tf.float16:
                new_z = spike_gauss_16(v_sc, self._gauss_std, self._dampening_factor)
            else:
                new_z = spike_gauss(v_sc, self._gauss_std, self._dampening_factor)
        else:
            if self.compute_dtype == tf.bfloat16:
                new_z = spike_function_b16(v_sc, self._dampening_factor)
            elif self.compute_dtype == tf.float16:
                new_z = spike_function_16(v_sc, self._dampening_factor)
            else:
                new_z = spike_function(v_sc, self._dampening_factor)

        # Generate the new spikes if the refractory period is concluded
        new_z = tf.where(new_r > 0.0, tf.zeros_like(new_z), new_z)
        # Add current spikes to the buffer
        new_z_buf = tf.concat([new_z, z_buf[:, :-self.n_neurons]], axis=1)  # Shift buffer

        # Define the model outputs and the new state of the network
        outputs = (
            new_z,
            new_v * self.voltage_scale + self.voltage_offset,
            (input_current + tf.reduce_sum(asc, axis=-1)) * self.voltage_scale,
        )

        new_state = (
            new_z_buf,
            new_v,
            new_r,
            new_asc,
            new_psc_rise,
            new_psc,
        )
        print('CALL DONE')
        return outputs, new_state
    
    def _convert_to_serializable(self, obj):
        print('_convert_to_serializable START')
        """Convert a nested structure with numpy arrays to lists"""
        if isinstance(obj, np.ndarray):
            return {"__ndarray__": True, "data": obj.tolist(), "dtype": str(obj.dtype)}
        elif isinstance(obj, dict):
            return {key: self._convert_to_serializable(value) for key, value in obj.items()}
        elif isinstance(obj, list):
            return [self._convert_to_serializable(item) for item in obj]
        elif isinstance(obj, tuple):
            return tuple(self._convert_to_serializable(item) for item in obj)
        elif isinstance(obj, (int, float, str, bool)) or obj is None:
            return obj
        elif isinstance(obj, np.generic):  # Handle numpy scalar types
            return obj.item()
        else:
            raise ValueError(f"Unable to serialize object of type {type(obj)}")
    
    @staticmethod
    def _convert_from_serializable(obj):
        """Convert serialized data back to numpy arrays where needed"""
        if isinstance(obj, dict):
            # First check if this is a serialized numpy array
            if obj.get("__ndarray__", False):
                return np.array(obj["data"], dtype=np.dtype(obj["dtype"]))
            
            # If not an array, recurse through dictionary
            return {key: V1Column._convert_from_serializable(value) 
                   for key, value in obj.items()}
        elif isinstance(obj, list):
            return [V1Column._convert_from_serializable(item) for item in obj]
        return obj
    
    # def print_datatypes(self, d):
    #     for key, value in d.items():
    #         if isinstance(value, dict):
    #             self.print_datatypes(value)
    #         else:
    #             print(f"{key}: {type(value).__name__}")
    # def print_network(self):  
    #     print('###Original network types###')        
    #     self.print_datatypes(self._network)
    
    def get_config(self):
        print('V1Column get_config called")')
        """Return configuration of the layer for serialization"""
        config = super().get_config()
               
        # create serializable metadata structure to store the dtype and shape of every value in self._network
        network_metadata = {}
        for key, value in self._network.items():
            if isinstance(value, np.ndarray):
                network_metadata[key] = {
                    'dtype': str(value.dtype),
                    'shape': value.shape
                }
            elif isinstance(value, dict):
                # Recursively handle nested dictionaries
                network_metadata[key] = {}
                for sub_key, sub_value in value.items():
                    if isinstance(sub_value, np.ndarray):
                        network_metadata[key][sub_key] = {
                            'dtype': str(sub_value.dtype),
                            'shape': sub_value.shape
                        }
                    else:
                        network_metadata[key][sub_key] = {
                            'dtype': str(type(sub_value).__name__),
                            'shape': None
                        }
            else:
                network_metadata[key] = {
                    'dtype': str(type(value).__name__),
                    'shape': None
                }
        config['network_metadata'] = network_metadata
        
        
        input_dict = {    
            'data_dir': self._data_dir,
            'dt': self._dt,
            'gauss_std': self._gauss_std,
            'dampening_factor': self._dampening_factor,
            'recurrent_dampening_factor': self._recurrent_dampening_factor,
            'input_weight_scale': self._input_weight_scale,
            'recurrent_weight_scale': self._recurrent_weight_scale,
            'lr_scale': self._lr_scale,
            'spike_gradient': self._spike_gradient,
            'max_delay': self._max_delay,
            'pseudo_gauss': self._pseudo_gauss,
            'train_recurrent': self._train_recurrent,
            'train_recurrent_per_type': self._train_recurrent_per_type,
            'train_input': self._train_input,
            'train_noise': self._train_noise,
            'hard_reset': self._hard_reset
        }
        
        # create a serializable metadata structure to store the dtype and shape of every value stored in the input_dict
        input_metadata = {}
        for key, value in input_dict.items():
            if isinstance(value, np.ndarray):
                input_metadata[key] = {
                    'dtype': str(value.dtype),
                    'shape': value.shape
                }
            else:
                input_metadata[key] = {
                    'dtype': str(type(value).__name__),
                    'shape': None
                }
        config['input_metadata'] = input_metadata
   
        # # store the dtype, and if available shape of every value in self._network
        # for key, value in self._network.items():    
        #     if isinstance(value, np.ndarray):
        #         config[f'{key}_dtype'] = str(value.dtype)
        #         config[f'{key}_shape'] = value.shape
        #     elif isinstance(value, dict):
        #         # Recursively handle nested dictionaries
        #         config[f'{key}_dtype'] = str(type(value).__name__)
        #         config[f'{key}_shape'] = None

        converted_network = self._convert_to_serializable(self._network)
        
        
        config.update({
            'network': converted_network,
            'lgn_input': self._convert_to_serializable(self._lgn_input),
            'bkg_input': self._convert_to_serializable(self._bkg_input),
            'data_dir': self._data_dir,
            'dt': self._dt,
            'gauss_std': self._gauss_std,
            'dampening_factor':self._dampening_factor,
            'recurrent_dampening_factor': self._recurrent_dampening_factor,
            'input_weight_scale': self._input_weight_scale,
            'recurrent_weight_scale': self._recurrent_weight_scale,
            'lr_scale': self._lr_scale,
            'spike_gradient': self._spike_gradient,
            'max_delay': self._max_delay,
            'pseudo_gauss': self._pseudo_gauss,
            'train_recurrent': self._train_recurrent,
            'train_recurrent_per_type': self._train_recurrent_per_type,
            'train_input': self._train_input,
            'train_noise': self._train_noise,
            'hard_reset': self._hard_reset,
        })
        return config
    
    @classmethod
    def from_config(cls, config):
        """Create an instance from configuration dictionary"""
        print("\n[DEBUG] Starting V1Column from_config reconstruction...")
        
        # Convert serialized arrays back to numpy arrays
        network = cls._convert_from_serializable(config['network'])
        
        # print("[DEBUG] Converted FROM serial network['node_params'] types:")
        # for key, value in network.items():
        #     print(f"[DEBUG] {key}: {type(value)}")
        #     if isinstance(value, np.ndarray):
        #         print(f"  - shape: {value.shape}, dtype: {value.dtype}")
        
        lgn_input = cls._convert_from_serializable(config['lgn_input'])
        bkg_input = cls._convert_from_serializable(config['bkg_input'])
        
        # Check that the network value types and shapes match the expected types saved in config['network_metadata']
        # metadata = config['network_metadata']
        # for key, value in network.items():
        #     if key not in metadata:
        #         raise ValueError(f"Key {key} not found in metadata")
        #     if isinstance(value, np.ndarray):
        #         if metadata[key]['dtype'] != str(value.dtype):
        #             print(f"Correcting data type for key {key}: expected {metadata[key]['dtype']}, got {value.dtype}")
        #             network[key] = value.astype(metadata[key]['dtype'])
        #         if metadata[key]['shape'] != value.shape:
        #             print(f"Correcting shape for key {key}: expected {metadata[key]['shape']}, got {value.shape}")
        #             network[key] = value.reshape(metadata[key]['shape'])
        #     elif isinstance(value, dict):
        #         for sub_key, sub_value in value.items():
        #             if sub_key not in metadata[key]:
        #                 raise ValueError(f"Sub-key {sub_key} not found in metadata for key {key}")
        #             if isinstance(sub_value, np.ndarray):
        #                 if metadata[key][sub_key]['dtype'] != str(sub_value.dtype):
        #                     print(f"Correcting data type for sub-key {sub_key} in key {key}: expected {metadata[key][sub_key]['dtype']}, got {sub_value.dtype}")
        #                     value[sub_key] = sub_value.astype(metadata[key][sub_key]['dtype'])
        #                     network[key][sub_key] = np.array(sub_value, dtype=metadata[key][sub_key]['dtype'])
        #                 if metadata[key][sub_key]['shape'] != sub_value.shape:
        #                     print(f"Correcting shape for sub-key {sub_key} in key {key}: expected {metadata[key][sub_key]['shape']}, got {sub_value.shape}")
        #                     value[sub_key] = sub_value.reshape(metadata[key][sub_key]['shape'])
        #                     network[key][sub_key] = np.array(sub_value, dtype=metadata[key][sub_key]['dtype'])
        #             else:
        #                 if metadata[key][sub_key]['dtype'] != str(type(sub_value).__name__):
        #                     print(f"Correcting data type for sub-key {sub_key} in key {key}: expected {metadata[key][sub_key]['dtype']}, got {type(sub_value).__name__}")
        #                     value[sub_key] = np.array(sub_value, dtype=metadata[key][sub_key]['dtype'])
        #                     network[key][sub_key] = np.array(sub_value, dtype=metadata[key][sub_key]['dtype'])
        #     else:
        #         if metadata[key]['dtype'] != str(type(value).__name__):
        #             print(f"Correcting data type for key {key}: expected {metadata[key]['dtype']}, got {type(value).__name__}")
        #             network[key] = np.array(value, dtype=metadata[key]['dtype'])
        
        print("[DEBUG] CORRECTED network types:")
        # for key, value in network.items():
        #     print(f"[DEBUG] {key}: {type(value)}")
        #     if isinstance(value, np.ndarray):
        #         print(f"  - shape: {value.shape}, dtype: {value.dtype}")
                
        # Check that the network value types and shapes match the expected types saved in config['input_metadata']
        input_keys = ['data_dir',
        'dt',
        'gauss_std',
        'dampening_factor',
        'recurrent_dampening_factor',
        'input_weight_scale',
        'recurrent_weight_scale',
        'lr_scale',
        'spike_gradient',
        'max_delay',
        'pseudo_gauss',
        'train_recurrent',
        'train_recurrent_per_type',
        'train_input',
        'train_noise',
        'hard_reset']
        
        # input_metadata = config['input_metadata']
        # for key in input_keys:
        #     if key in config:
        #         expected_dtype = input_metadata[key]['dtype']
        #         if isinstance(config[key], np.ndarray):
        #             if expected_dtype != str(config[key].dtype):
        #                 print(f"Correcting data type for key {key}: expected {expected_dtype}, got {config[key].dtype}")
        #                 config[key] = config[key].astype(expected_dtype)
        #             expected_shape = input_metadata[key]['shape']
        #             if expected_shape and expected_shape != config[key].shape:
        #                 print(f"Correcting shape for key {key}: expected {expected_shape}, got {config[key].shape}")
        #                 config[key] = config[key].reshape(expected_shape)
        #         else:
        #             if expected_dtype != str(type(config[key]).__name__):
        #                 print(f"Correcting data type for key {key}: expected {expected_dtype}, got {type(config[key]).__name__}")
        #                 config[key] = np.array(config[key], dtype=expected_dtype)
        
        print("[DEBUG] CORRECTED serial input types:")
        # for key in input_keys:
        #     value = config[key]
        #     print(f"[DEBUG] {key}: {type(value)}")
        #     if isinstance(value, np.ndarray):
        #         print(f"  - shape: {value.shape}, dtype: {value.dtype}")
                
        # Create instance with converted data
        return cls(
            network=network,
            lgn_input=lgn_input,
            bkg_input=bkg_input,
            data_dir=config['data_dir'],
            dt=config['dt'],
            gauss_std=config['gauss_std'],
            dampening_factor=config['dampening_factor'],
            recurrent_dampening_factor=config['recurrent_dampening_factor'],
            input_weight_scale=config['input_weight_scale'],
            recurrent_weight_scale=config['recurrent_weight_scale'],
            lr_scale=config['lr_scale'],
            spike_gradient=config['spike_gradient'],
            max_delay=config['max_delay'],
            pseudo_gauss=config['pseudo_gauss'],
            train_recurrent=config['train_recurrent'],
            train_recurrent_per_type=config['train_recurrent_per_type'],
            train_input=config['train_input'],
            train_noise=config['train_noise'],
            hard_reset=config['hard_reset'],
        )

    def get_weights(self):
        print("""Override get_weights to include custom weights""")
        weights = super().get_weights()
        # Add custom weights as needed
        custom_weights = []
        if hasattr(self, 'recurrent_weight_values'):
            custom_weights.append(self.recurrent_weight_values.numpy())
        if hasattr(self, 'input_weight_values'):
            custom_weights.append(self.input_weight_values.numpy())
        if hasattr(self, 'bkg_input_weights'):
            custom_weights.append(self.bkg_input_weights.numpy())
        return weights + custom_weights

    def set_weights(self, weights):
        print("""Override set_weights to handle custom weights""")
        # Handle base layer weights
        n_base_weights = len(super().get_weights())
        super().set_weights(weights[:n_base_weights])
        
        # Handle custom weights
        custom_weights = weights[n_base_weights:]
        if hasattr(self, 'recurrent_weight_values'):
            self.recurrent_weight_values.assign(custom_weights[0])
        if hasattr(self, 'input_weight_values'):
            self.input_weight_values.assign(custom_weights[1])
        if hasattr(self, 'bkg_input_weights'):
            self.bkg_input_weights.assign(custom_weights[2])
    
    # def get_config(self):
    #     # Get base config from parent class
    #     base_config = super().get_config()
        
    #     # Store all initialization parameters
    #     config = {
    #         # Network parameters
    #         'network': {
    #             'node_params': {
    #                 'V_th': self.V_th.tolist(),
    #                 'E_L': self.E_L.tolist(),
    #                 'V_reset': self.V_reset.tolist(),
    #                 'asc_amps': self.asc_amps.tolist(),
    #                 'g': self.g.tolist(),
    #                 'k': self.k.tolist(),
    #                 't_ref': self.t_ref.tolist(),
    #                 'C_m': self.C_m.tolist(),                    
    #             },
    #             'node_type_ids': self.node_type_ids.tolist(),
    #             'n_nodes': self.n_neurons,
    #             'synapses': {
    #                 'indices': self.indices.tolist(),
    #                 'weights': self.recurrent_weights.tolist(),
    #                 'dense_shape': self.dense_shape.tolist(),
    #                 'syn_ids': self.syn_ids.numpy().tolist(),
    #                 'delays': self.delays.tolist()
    #             }
    #         },
    #         # LGN input parameters
    #         'lgn_input': {
    #             'indices': self._input_indices.tolist(),
    #             'weights': self.input_weights.tolist(),
    #             'syn_ids': self._input_syn_ids.tolist(),
    #             'delays': self.input_delays.tolist(),
    #             'n_inputs': self.lgn_input_dense_shape[1]
    #         },
    #         # Background input parameters
    #         'bkg_input': {
    #             'indices': self._bkg_input_indices.tolist(),
    #             'weights': self._bkg_input_weights.tolist(),
    #             'syn_ids': self._bkg_input_syn_ids.tolist(),
    #             'delays': self.bkg_input_delays.tolist(),
    #             'n_inputs': self.bkg_input_dense_shape[1]
    #         },
    #         # Configuration parameters
    #         'dt': float(self._dt.numpy()),
    #         'gauss_std': float(self._gauss_std.numpy()),
    #         'dampening_factor': float(self._dampening_factor.numpy()),
    #         'recurrent_dampening_factor': float(self._recurrent_dampening.numpy()),
    #         'input_weight_scale': self.input_weight_scale,  
    #         'recurrent_weight_scale': self.recurrent_weight_scale, 
    #         'lr_scale': float(self._lr_scale.numpy()),
    #         'spike_gradient': self._spike_gradient,
    #         'max_delay': self._max_delay,
    #         'pseudo_gauss': self._pseudo_gauss,
    #         'train_recurrent': self._train_recurrent,
    #         'train_recurrent_per_type': self._train_recurrent_per_type,
    #         'train_input': self._train_input,
    #         'train_noise': self._train_noise,
    #         'hard_reset': self._hard_reset,
    #         # Additional parameters
    #         'data_dir': self._data_dir
    #         #'synaptic_basis_weights': self.synaptic_basis_weights.numpy().tolist()
    #     }
    #     return {**base_config, **config}

    # @classmethod
    # def from_config(cls, config):
    #     # Extract network, lgn_input, and bkg_input from config
    #     print('extract network config')
    #     network = config.pop('network')
    #     lgn_input = config.pop('lgn_input')
    #     bkg_input = config.pop('bkg_input')
    #     print('convert to numpy arrays')
    #     # Convert lists back to numpy arrays where needed
    #     network['node_params']['V_th'] = np.array(network['node_params']['V_th'])
    #     network['node_params']['E_L'] = np.array(network['node_params']['E_L'])
    #     network['node_params']['V_reset'] = np.array(network['node_params']['V_reset'])
    #     network['node_params']['asc_amps'] = np.array(network['node_params']['asc_amps'])
    #     network['node_params']['g'] = np.array(network['node_params']['g'])
    #     network['node_params']['k'] = np.array(network['node_params']['k'])
    #     network['node_params']['t_ref'] = np.array(network['node_params']['t_ref'])
    #     network['node_params']['C_m'] = np.array(network['node_params']['C_m'])                  
                
    #     network['node_type_ids'] = np.array(network['node_type_ids'])
    #     network['synapses']['indices'] = np.array(network['synapses']['indices'])
    #     network['synapses']['weights'] = np.array(network['synapses']['weights'])
    #     network['synapses']['syn_ids'] = np.array(network['synapses']['syn_ids'])
        
    #     lgn_input['indices'] = np.array(lgn_input['indices'])
    #     lgn_input['weights'] = np.array(lgn_input['weights'])
    #     lgn_input['syn_ids'] = np.array(lgn_input['syn_ids'])
        
    #     bkg_input['indices'] = np.array(bkg_input['indices'])
    #     bkg_input['weights'] = np.array(bkg_input['weights'])
    #     bkg_input['syn_ids'] = np.array(bkg_input['syn_ids'])
        
    #     # Convert synaptic_basis_weights back to numpy array
    #     #config['synaptic_basis_weights'] = np.array(config.pop('synaptic_basis_weights'))
        
    #     # Create new instance with recovered parameters
    #     return cls(
    #         network=network,
    #         lgn_input=lgn_input,
    #         bkg_input=bkg_input,
    #         **config
    #     )

# # pilot test of RNN cell wrapper to use the V1Column class to allow for proper serialization to save & load model
@tf.keras.utils.register_keras_serializable(package='Custom')
class V1ColumnRNNCell(tf.keras.layers.AbstractRNNCell):
    def __init__(self, v1_column, **kwargs):
        print(f"[DEBUG] Initializing V1ColumnRNNCell")
        super().__init__(**kwargs)
        self.v1_column = v1_column
        self.supports_constants = True

    @property
    def state_size(self):
        size = self.v1_column.state_size
        print(f"[DEBUG] Getting state_size")
        return size

    def get_initial_state(self, inputs=None, batch_size=None, dtype=None):
        print(f"[DEBUG] Creating initial state")
        return self.v1_column.zero_state(batch_size, dtype)

    def call(self, inputs, states, constants=None, training=None):
        print(f"[DEBUG] Executing call method")
        return self.v1_column(inputs, states, constants=constants)
    
    #@classmethod
    def get_config(self):
        print("\n[DEBUG] V1ColumnRNNCell get_config called")
        try:
            base_config = super().get_config()
            print("[DEBUG] ✓ Got base config")
            
            v1_config = self.v1_column.get_config()
            print("[DEBUG] ✓ Got V1Column config")
            
            config = {
                **base_config,
                'v1_column_config': v1_config
            }
            print("[DEBUG] ✓ Created final config")
            #return config
            return {**base_config, **config}
            
        except Exception as e:
            print(f"[DEBUG] ✗ Error in get_config: {type(e).__name__}")
            print(f"[DEBUG] ✗ Error message: {str(e)}")
            raise

    @classmethod
    def from_config(cls, config):
        print("\n[DEBUG] Starting V1ColumnRNNCell from_config")
        try:
            v1_config = config.pop('v1_column_config')
            print(f"[DEBUG] ✓ Extracted V1Column config keys: {list(v1_config.keys())}")
            print("[DEBUG] ✓ V1Column config types:")
            for key, value in v1_config.items():
                print(f"    - {key}: {type(value)}")
                
            print("[DEBUG] ✓ Converting V1Column config types:")
            v1_column = V1Column.from_config(v1_config)
            # ## need to call V1Column from config method here to recreate the V1Column inputs?
            # # I don't think V1Column from config is ever being called to deserialize things
            # v1_config = self.v1_column.from_config(v1_config)
            #print("\n[DEBUG] Creating V1Column with config...")
            #v1_column = V1Column(**v1_config)
            print("[DEBUG] ✓ Created V1Column instance")
            
            instance = cls(v1_column, **config)
            print("[DEBUG] ✓ Created cell instance")
            return instance
            
        except Exception as e:
            print(f"\n[DEBUG] ✗ Error in from_config: {type(e).__name__}")
            print(f"[DEBUG] ✗ Error message: {str(e)}")
            print("[DEBUG] ✗ Error location:", end=" ")
            if 'v1_config' not in locals():
                print("Failed to extract V1Column config")
            elif 'v1_column' not in locals():
                print("Failed to create V1Column instance")
                # Add more detailed error info for V1Column creation
                import traceback
                print("\n[DEBUG] Detailed error information:")
                print(traceback.format_exc())
            else:
                print("Failed to create cell instance")
            raise

# @profile
def create_model(
    network,
    lgn_input,
    bkg_input,
    data_dir = "GLIF_network",
    seq_len=100,
    n_input=10,
    n_output=2,
    cue_duration=20,
    dtype=tf.float32,
    input_weight_scale=1.0,
    gauss_std=0.5,
    dampening_factor=0.2,
    recurrent_dampening_factor=0.5,
    lr_scale=800.0,
    train_recurrent=True,
    train_recurrent_per_type=False,
    train_input=True,
    train_noise=True,
    neuron_output=False,
    use_state_input=False,
    return_state=False,
    return_sequences=False,
    down_sample=50,
    add_metric=True,
    max_delay=5,
    batch_size=None,
    pseudo_gauss=False,
    hard_reset=False,
):

    # Create the input layer of the model
    x = tf.keras.layers.Input(shape=(None, n_input,))
    neurons = network["n_nodes"]

    # Create an input layer for the initial state of the RNN
    state_input_holder = tf.keras.layers.Input(shape=(None, neurons))
    state_input = tf.cast(tf.identity(state_input_holder), dtype)  

    # If batch_size is not provided as an argument, it is automatically inferred from the
    # first dimension of x using tf.shape().
    if batch_size is None:
        batch_size = tf.shape(x)[0]
    else:
        batch_size = batch_size

    # Create the V1Column cell
    print('Creating the V1 column...')
    # time0 = time()
    cell = V1Column(
        network,
        lgn_input,
        bkg_input,
        data_dir=data_dir,
        gauss_std=gauss_std,
        dampening_factor=dampening_factor,
        input_weight_scale=input_weight_scale,
        lr_scale=lr_scale,
        spike_gradient=True,
        recurrent_dampening_factor=recurrent_dampening_factor,
        max_delay=max_delay,
        pseudo_gauss=pseudo_gauss,
        train_recurrent=train_recurrent,
        train_recurrent_per_type=train_recurrent_per_type,
        train_input=train_input,
        train_noise=train_noise,
        hard_reset=hard_reset,
    )
    # initialize the RNN state to zero using the zero_state() method of the V1Column class.
    zero_state = cell.zero_state(batch_size, dtype)

    if use_state_input:
        # The shape of each input tensor matches the shape of the corresponding
        # tensor in the zero_state tuple, except for the batch dimension. The batch
        # dimension is left unspecified, allowing the tensor to be fed variable-sized
        # batches of data.
        initial_state_holder = tf.nest.map_structure(lambda _x: tf.keras.layers.Input(shape=_x.shape[1:], dtype=_x.dtype), zero_state)
        # The code then copies the input tensors into the rnn_initial_state variable
        # using tf.nest.map_structure(). This creates a nested structure of tensors with
        # the same shape as the original zero_state structure.
        rnn_initial_state = tf.nest.map_structure(tf.identity, initial_state_holder)
        # In both cases, the code creates a constants tensor using tf.zeros_like() or
        # tf.zeros(). This tensor is used to provide constant input to the RNN during
        # computation. The shape of the constants tensor matches the batch_size.
        constants = tf.zeros_like(rnn_initial_state[0][:, 0], dtype)
    else:
        rnn_initial_state = zero_state
        constants = tf.zeros((batch_size,))
    # Create the LGN input layer of the model
    lgn_inputs = LGNInputLayer(
        cell.input_indices,
        cell.input_weight_values,
        cell.lgn_input_dense_shape,
        cell.input_syn_ids,
        cell.synaptic_basis_weights,
        # lr_scale=lr_scale,
        name="input_layer",
    )(x)

    # # Create the BKG input layer of the model
    # bkg_inputs = BackgroundNoiseLayer(
    #     cell.bkg_input_indices,
    #     cell.bkg_input_weights,
    #     cell.bkg_input_dense_shape,
    #     cell.bkg_input_weights_factors, 
    #     batch_size, 
    #     seq_len,
    #     # lr_scale=lr_scale,
    #     name="noise_layer",
    # )(x) # the input is provided just because in a Keras custom layer, the call method should accept input

    # Generate the background spikes
    # seq_len = tf.shape(x)[1]
    bkg_inputs = BKGInputLayer(
        cell.bkg_input_indices,
        cell.bkg_input_weights,
        cell.bkg_input_dense_shape,
        cell.bkg_input_syn_ids, 
        cell.synaptic_basis_weights,
        batch_size, 
        name="noise_layer",
        )(x)

    # Concatenate the input layer with the initial state of the RNN
    full_inputs = tf.concat((lgn_inputs, bkg_inputs, state_input), -1) # (None, 600, 5*n_neurons+n_neurons)
    
    # Create the RNN layer of the model using the V1Column cell
    # The RNN layer returns the output of the RNN layer and the final state of the RNN
    # layer. The output of the RNN layer is a tensor of shape (batch_size, seq_len,
    # neurons). The final state of the RNN layer is a tuple of tensors, each of shape
    # (batch_size, neurons).
    rnn_cell = V1ColumnRNNCell(cell)
    print('Creating the rsnn layer...')
    rnn = tf.keras.layers.RNN(rnn_cell, return_sequences=True, return_state=return_state, name="rsnn")
    # rnn = tf.keras.layers.RNN(cell, return_sequences=True, return_state=return_state, name="rsnn")

    # Apply the rnn layer to the full_inputs tensor
    out = rnn(full_inputs, initial_state=rnn_initial_state, constants=constants)

    # Check if the return_state argument is True or False and assign the output of the
    # RNN layer to the hidden variable accordingly.
    if return_state:
        hidden = out[0]
        # new_state = out[1:]
    else:
        hidden = out

    spikes = hidden[0]
    voltage = hidden[1]

    # computes the mean of the spikes tensor along the second and third dimensions
    # (which represent time and neurons),
    rate = tf.reduce_mean(spikes, (1, 2))

    # The neuron output option selects only the output neurons from the spikes tensor
    if neuron_output:
        # The output_spikes tensor is computed by taking a linear combination
        # of the current spikes and the previous spikes, with the coefficients
        # determined by the dampening_factor. This serves to reduce the volatility
        # of the output spikes, making them more stable.
        output_spikes = 1 / dampening_factor * spikes + (1 - 1 / dampening_factor) * tf.stop_gradient(spikes)
        # The output tensor is then computed by selecting the spikes from the
        # output neurons and scaling them by a learned factor. The scale factor
        # is computed using a softplus activation function applied to the output
        # of a dense layer, and the threshold is computed by passing the output
        # spikes through another dense layer.
        output = tf.gather(output_spikes, network["readout_neuron_ids"], axis=2)
        output = tf.reduce_mean(output, -1)
        scale = 1 + tf.nn.softplus(tf.keras.layers.Dense(1)(tf.zeros_like(output[:1, :1])))
        thresh = tf.keras.layers.Dense(1)(tf.zeros_like(output))
        output = tf.stack([thresh[..., 0], output[..., -1]], -1) * scale
    # If neuron_output is False, then the output tensor is simply the result of
    # passing the spikes tensor through a dense layer with n_output units.
    else:
        output = tf.keras.layers.Dense(n_output, name="projection", trainable=False)(spikes)
        # output = tf.keras.layers.Dense(n_output, name="projection", trainable=True)(spikes)
        
    # Finally, a prediction layer is created
    output = tf.keras.layers.Lambda(lambda _a: _a, name="prediction")(output)

    # If return_sequences is True, then the mean_output tensor is computed by
    # averaging over sequences of length down_sample in the output tensor.
    # Otherwise, mean_output is simply the mean of the last cue_duration time steps
    # of the output tensor.
    # if return_sequences:
    #     mean_output = tf.reshape(output, (-1, int(seq_len / down_sample), down_sample, n_output))
    #     mean_output = tf.reduce_mean(mean_output, 2)
    #     mean_output = tf.nn.softmax(mean_output, axis=-1)
    # else:
    #     mean_output = tf.reduce_mean(output[:, -cue_duration:], 1)
    #     mean_output = tf.nn.softmax(mean_output)

    if use_state_input:
        # many_input_model = tf.keras.Model(
        #     inputs=[x, state_input_holder, initial_state_holder], 
        #     outputs=mean_output
        # )
        many_input_model = tf.keras.Model(
            inputs=[x, state_input_holder,initial_state_holder],
            outputs=[output])
    else:
        # many_input_model = tf.keras.Model(
        #     inputs=[x, state_input_holder], 
        #     outputs=mean_output
        # )
        many_input_model = tf.keras.Model(
            inputs=[x, state_input_holder],
            outputs=[output])

    if add_metric:
        # add the firing rate of the neurons as a metric to the model
        many_input_model.add_metric(rate, name="rate")

    return many_input_model


# if name is main run the create model function
if __name__ == "__main__":
    # load the network
    import load_sparse

    n_input = 17400
    n_neurons = 1574

    network, lgn_input, bkg_input = load_sparse.cached_load_v1(
        n_input,
        n_neurons,
        True,
        "GLIF_network",
        seed=3000,
        connected_selection=False,
        n_output=2,
        neurons_per_output=16,
    )
    # create the model
    model = create_model(
        network,
        lgn_input,
        bkg_input,
        data_dir = "GLIF_network",
        seq_len=100,
        n_input=n_input,
        n_output=2,
        cue_duration=20,
        dtype=tf.float32,
        input_weight_scale=1.0,
        gauss_std=0.5,
        dampening_factor=0.2,
        lr_scale=800.0,
        train_recurrent=True,
        train_recurrent_per_type=False,
        train_input=True,
        neuron_output=False,
        recurrent_dampening_factor=0.5,
        use_state_input=False,
        return_state=False,
        return_sequences=False,
        down_sample=50,
        add_metric=True,
        max_delay=5,
        batch_size=1,
        pseudo_gauss=False,
        hard_reset=True,
    )

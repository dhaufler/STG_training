...
class StiffRegularizer(Layer):
    """
    A custom TensorFlow layer that implements a regularizer to penalize deviations in synaptic weights.
    Attributes:
        _strength (tf.Tensor): The strength of the regularization.
        _dtype (tf.DType): The data type for TensorFlow operations.
        _penalize_relative_change (bool): Flag to determine if relative changes should be penalized.
        idx (tf.Tensor): Tensor containing indices for unique edge types.
        num_unique (tf.Tensor): Tensor containing the number of unique edge types.
        _target_mean_weights (tf.Tensor): Tensor containing the target mean weights for each edge type.
        _denominator (tf.Tensor): Tensor containing denominators for relative deviation calculation (if penalize_relative_change is True).
    Methods:
        __init__(self, strength, network, penalize_relative_change=False, dtype=tf.float32):
            Initializes the StiffRegularizer with the given parameters and computes initial values.
        __call__(self, x):
            Computes the regularization loss for the given input tensor x.
    """
    ...
    def __call__(self, x):
        """
        Computes the regularization loss based on the input tensor `x`.
        Parameters:
        x (tf.Tensor): Input tensor which can be of any shape. If the tensor has more than one dimension and the second dimension is 1, it will be squeezed.
        Returns:
        tf.Tensor: The computed regularization loss, cast to the specified dtype and scaled by the strength attribute.
        The function performs the following steps:
        1. If the input tensor `x` has more than one dimension and the second dimension is 1, it squeezes the tensor.
        2. Computes the mean edge type weights using `tf.math.unsorted_segment_mean`.
        3. If the `_penalize_relative_change` attribute is True, it calculates the relative deviation and computes the regularization loss as the square root of the mean squared relative deviation.
        4. If `_penalize_relative_change` is False, it computes the regularization loss as the mean squared difference between the mean edge type weights and the target mean weights.
        5. Casts the regularization loss to the specified dtype and scales it by the strength attribute.
        """
...
class L2Regularizer(tf.keras.regularizers.Regularizer):
    """
    A custom L2 regularizer for TensorFlow Keras models that can optionally penalize relative changes in weights.
    Args:
        strength (float): The regularization strength.
        network (dict): A dictionary containing network parameters and synapse information.
        flags (dict): Additional flags for configuration.
        penalize_relative_change (bool, optional): If True, penalizes relative changes in weights. Defaults to False.
        dtype (tf.DType, optional): The data type for the regularizer. Defaults to tf.float32.
    Attributes:
        _strength (tf.Tensor): The regularization strength cast to the specified dtype.
        _dtype (tf.DType): The data type for the regularizer.
        _penalize_relative_change (bool): Whether to penalize relative changes in weights.
        _target_mean_weights (tf.Tensor or None): The target mean weights for each edge type, or None if not penalizing relative changes.
    Methods:
        __call__(x):
            Computes the regularization penalty for the given weights.
            Args:
                x (tf.Tensor): The weights to regularize.
            Returns:
                tf.Tensor: The computed regularization penalty.
    """

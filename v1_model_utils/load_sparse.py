...
def create_network_dat(data_dir='GLIF_network/network', source='stg', target='stg', 
    ...
    # If the source and target are stg, then extract nodes dynamic parameters
    if source == "stg" and target == "stg":
        ...
        for node_type_id in node_types_df["node_type_id"]:
            ...
            #node_params_file = os.path.join(cell_models_path, f"{model_type_id}_glif_lif_asc_config.json")
...
def load_network(
    ...
    if n_neurons > n_nodes:  # used to be an explicit number: 296991
        ...
        print('Number nodes available: ', n_nodes)
        print('Number neurons to select: ', n_neurons)
        ...
        # !!! np.empty creates an array with a RANDOM amount of True & False values, changed to np.zeros !!!
...
# @profile
def load_stg(flags, n_neurons, flag_str=''):
    # Initialize the network 
    t0 = time()
    network = load_network(
                        path=os.path.join(flags.net_dir, "network_dat.pkl"),
                        h5_path=os.path.join(flags.net_dir, "network/stg_nodes.h5"),
    ...
    df = pd.read_csv(os.path.join(
        flags.net_dir, "network/stg_node_types.csv"), delimiter=" ")
    ...
    network["data_dir"] = flags.net_dir # to use later
    #########################################
    ...
    inputs = load_input(
                        ...
                        duration=1000,
                        dt=1,
                        data_dir=os.path.join(flags.net_dir, "network"),
                        lgn_path=os.path.join(flags.net_dir, "aud_input_dat.pkl"),
                        bkg_path=os.path.join(flags.net_dir, "bkg_input_dat.pkl"),
...
def cached_load_stg(flags, n_neurons, flag_str=''):
    store = False
    network, lgn_input, bkg_input = None, None, None

    if flag_str == '':
        flag_str = f"neurons_{n_neurons}_n_input_{flags.n_input}_s{flags.seed}_c{flags.core_only}_con{flags.connected_selection}_{flags.net_dir.replace('/', '_')}"
    
    file_dir = os.path.split(__file__)[0]
    cache_path = os.path.join(file_dir, f".cache/stg_network_{flag_str}.pkl")
    print(f"> Looking for cached STG model in {cache_path}")
    
    if os.path.exists(cache_path):
        try:
            with open(cache_path, "rb") as f:
                network, lgn_input, bkg_input = pkl.load(f)
                print(f"> Sucessfully restored STG model from {cache_path}")
    ...
    if lgn_input is None or network is None or bkg_input is None:
        network, lgn_input, bkg_input = load_stg(flags=flags, n_neurons=n_neurons)

    if store:
        os.makedirs(os.path.join(file_dir, ".cache"), exist_ok=True)
        with open(cache_path, "wb") as f:
            pkl.dump((network, lgn_input, bkg_input), f)
        print(f"> Cached STG model in {cache_path}")
